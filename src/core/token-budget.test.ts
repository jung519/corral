...
describe('sharing one counter', () => {
  it('lets spending recorded by either side close the door for both', () => {
    ...
    b.record({ inputTokens: 45, outputTokens: 0 }, 'development');
  ...
  it('stops on the output ceiling too', () => {
    ...
    b.record({ inputTokens: 99_999, outputTokens: 100 }, 'development');
  ...
  it('never blocks when no ceiling is configured', () => {
    ...
    b.record({ inputTokens: 10_000_000, outputTokens: 10_000_000 }, 'development');
...
describe('the tally surviving a restart', () => {
  it('is not handed out twice', () => {
    budget({ dailyInputTokens: 100 }).record({ inputTokens: 90, outputTokens: 0 }, 'development');
    ...
    budget({ dailyInputTokens: 100 }).record({ inputTokens: 20, outputTokens: 0 }, 'development');
    expect(budget({ dailyInputTokens: 100 }).check().ok).toBe(false);
  });

  it('starts fresh on a new day', () => {
    budget({ dailyInputTokens: 100 }).record({ inputTokens: 200, outputTokens: 0 }, 'development');
    expect(budget({ dailyInputTokens: 100 }).check().ok).toBe(false);
  ...
  it('rolls over mid-process, not just on restart', () => {
    ...
    b.record({ inputTokens: 200, outputTokens: 0 }, 'development');
...
describe('telling someone', () => {
  ...
  it('announces each threshold once, on the call that crosses it', () => {
    ...
    b.record({ inputTokens: 20, outputTokens: 0 }, 'development'); // 20%
    expect(notices()).toHaveLength(0);

    b.record({ inputTokens: 35, outputTokens: 0 }, 'development'); // 55%
    b.record({ inputTokens: 5, outputTokens: 0 }, 'development'); // 60% — still past 50 only
    expect(notices().map((e) => e.data?.threshold)).toEqual([50]);

    b.record({ inputTokens: 25, outputTokens: 0 }, 'development'); // 85%
    b.record({ inputTokens: 20, outputTokens: 0 }, 'development'); // 105%
  ...
  it('does not repeat itself once the limit is spent', () => {
    ...
    for (let i = 0; i < 20; i++) b.record({ inputTokens: 10, outputTokens: 0 }, 'development');
  ...
  it('raises the last one as an error, not a passing remark', () => {
    ...
    b.record({ inputTokens: 10, outputTokens: 0 }, 'development');

    expect(notices().at(-1)).toMatchObject({ kind: 'error' });
  });

  it('says nothing at all when no ceiling is configured', () => {
    budget({}).record({ inputTokens: 999_999, outputTokens: 999_999 }, 'development');
...
describe('what it reports', () => {
  it('gives the fraction of the tightest ceiling', () => {
    ...
    b.record({ inputTokens: 100, outputTokens: 50 }, 'development');
  ...
  it('writes the day and the tally where a restart can find them', () => {
    budget().record({ inputTokens: 7, outputTokens: 3 }, 'development');
