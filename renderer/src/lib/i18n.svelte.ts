...
const en: Dict = {
  ...
  'workspace.mountLoginOn': 'Mounts ~/.claude (read-only) so the agent authenticates like local — no API key needed.',
  'workspace.mountLoginOff': 'Off → the agent uses an API key instead (required in step 1).',
...
const ko: Dict = {
  ...
  'workspace.mountLoginOn': '~/.claude를 읽기 전용으로 마운트해 local처럼 인증 — API 키 불필요.',
  'workspace.mountLoginOff': '끄면 → 에이전트가 API 키를 사용(1단계에서 필수).',
  'badge.soon': '준비 중',
