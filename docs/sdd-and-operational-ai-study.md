...
- **탈출구를 둔다** → 복잡한 오퍼레이션은 코드(플러그인)로, corral은 실행·폴오버·상한·관측만 제공

corral은 후자의 골격을 이미 갖고 있다(`Registry` 패턴 + `@corral/sdk` 노출 계획, 5축이 전부
"인터페이스 + 레퍼런스 1~2종" 구조). **선언형을 1차로 두되 처음부터 코드 탈출구를 같이 설계**하는
쪽을 권한다. 그 탈출구 코드는 개발AI가 SDD로 만들어줄 수 있다.
...
설정 UI의 모양, 이력에 남길 항목, 검증 층의 위치가 전부 여기서 갈린다. 답이 "둘 다, 선언을
기본으로"라면 **선언 스키마가 코드 오퍼레이션의 부분집합이 되도록** 처음부터 잡아야 나중에
마이그레이션이 안 생긴다.
