...
> 작업 계획: `ops-prompt-context-plan.md` · 이슈 CRL-63·64·65·66·67·68
...
| 호출 빈도 (§6-3) | 정상 하루 수 건, 발행 누락 시 최대 200건. 1건당 약 800토큰 |

**§3이 지적한 "성공으로 끝나는 실패"는 별도로 먼저 고쳤다** — 전부 버려지면 `rejected`다
(CRL-63). 근본 원인과 독립적이라 `context` 결정을 기다리지 않았다.

**§6-3의 전제가 틀렸었다.** "목록을 프롬프트에 넣으면 토큰이 는다"고 봤는데, refinery는
이미 넣고 있다(`classify-festival.operation.ts`). 그 상태의 실측이 1건 약 800토큰이므로
이 작업은 비용을 늘리는 것이 아니라 corral이 안 넣는 예외 상태를 없애는 것이었다.
